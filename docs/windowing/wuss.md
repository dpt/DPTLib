...
- `wuss_COLOUR_BLACK`, `_WHITE`, `_RED`, `_GREEN`, `_BLUE`, `_YELLOW`, `_CYAN`, `_MAGENTA`, `_GREY`, `_WINDOW` (light grey), `_MENU` (white) — nearest system-palette entry to the named RGB, the same lookup as `wuss_nearest_colour`. `_WINDOW` and `_MENU` name the conventional window-body and menu-body fills.
