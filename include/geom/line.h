 ...
 * in `x0` and co.
 *
 * The returned points are rounded to the nearest integer position on the clip
 * boundary, so they vary with the clip box given. Callers which step along the
 * line incrementally (Bresenham, Wu, etc.) must not seed their error terms from
 * them if the pixels drawn are to be independent of the clip box passed in; use
 * the return value to reject and step from the original endpoints.
 *
